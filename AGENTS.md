...
- **Naming**: module folders must be `azurerm_<resource>`, `azuredevops_<resource>`, or `kubernetes_<resource>` when the module's primary scope is in-cluster Kubernetes resources managed through the `kubernetes` provider.
