# TASK-039: Azure Windows Virtual Machine module (full feature scope)
# FileName: TASK-039_Azure_Windows_Virtual_Machine_Module.md
...
Stworzyc nowy modul `modules/azurerm_windows_virtual_machine` zgodny z:
- `docs/MODULE_GUIDE/`
- `docs/TESTING_GUIDE/`
- `docs/TERRAFORM_BEST_PRACTICES_GUIDE.md`
- `docs/SECURITY.md`

Modul ma pokrywac wszystkie funkcje dostepne w `azurerm_windows_virtual_machine`
...
**Potwierdzone w providerze (azurerm 4.57.0, do potwierdzenia w TASK-039-1):**
- `azurerm_windows_virtual_machine`
- `azurerm_virtual_machine_extension`
- `azurerm_monitor_diagnostic_setting`
...
1) **Atomic scope**  
   Modul zarzadza jednym Windows VM jako primary resource. Dodatkowe zasoby tylko
...
2) **Brak cross-resource glue**  
   ...
   Log Analytics workspace, monitoring glue, Azure Bastion. Pokazujemy je tylko
   w examples jako osobne zasoby.

3) **Security-first**  
   Secure defaults gdzie to mozliwe (silne haslo, ograniczony RDP, brak public IP
...
- Windows profile: computer_name, timezone, winrm_listener,
  additional_unattend_content
- Secrets: Key Vault certificates (jesli wspierane)
- License: license_type
- Plan / purchase plan (marketplace images)
- Custom data / user data (jesli wspierane)
- Tags i timeouts
- VM extensions
...
### TASK-039-1: Discovery / feature inventory

**Cel:** Potwierdzic pelny zakres funkcji provider `azurerm` dla Windows VM.

**Do zrobienia:**
- Sprawdzic schema provider `azurerm` (doc lub `terraform providers schema -json`)
  dla `azurerm_windows_virtual_machine`.
- Potwierdzic dokladne pola i zaleznosci dla:
  - admin username/password + wymagania Windows
  ...
  - patching/agent + automatic updates
  - plan / marketplace images
  - windows profile (computer_name, timezone, winrm_listener, unattend)
  - secrets (Key Vault certs) i license_type
...
### TASK-039-2: Scaffold modulu + pliki bazowe

**Cel:** Stworzyc pelna strukture modulu zgodna z guide.

**Checklist:**
- [ ] `modules/azurerm_windows_virtual_machine/` utworzony przez
      `scripts/create-new-module.sh` (wymagane).
- [ ] `module.json`:
  - name: `azurerm_windows_virtual_machine`
  - commit_scope: `windows-virtual-machine`
  - tag_prefix: `WINDOWSVMv`
...
### TASK-039-3: Core resource `azurerm_windows_virtual_machine`

**Cel:** Implementacja pelnego API Windows VM w `main.tf` + walidacje w `variables.tf`.
...
- **agent/patching**: `provision_vm_agent`, `patch_mode`, `patch_assessment_mode`,
  `enable_automatic_updates`
- **windows_profile**: `computer_name`, `timezone`, `winrm_listener`,
  `additional_unattend_content`
- **secrets**: `secret` (Key Vault certificates)
- **license**: `license_type`
...
- `admin_username` i `admin_password` wymagane; blokada dla reserved usernames
  wg zasad Windows.
- `computer_name` zgodny z limitami Windows (length/charset).
...
- `winrm_listener` i `additional_unattend_content` zgodnie z schema.
- `secrets` tylko gdy podany Key Vault i wymagane pola.
...
### TASK-039-4: Sub-resources (VM extensions)

**Cel:** Pelne wsparcie VM extensions powiazanych z Windows VM.
...
### TASK-039-5: Diagnostic settings (inline, AKS pattern)
...
### TASK-039-6: Dokumentacja modulu
...
- `SECURITY.md`:
  - RDP exposure, hasla, winrm/secret handling, boot diagnostics data
  - secure example (no public IP, ograniczony RDP)
  - checklist i typowe bledy
- `CONTRIBUTING.md` i `VERSIONING.md` zgodne z `module.json`.

---

### TASK-039-7: Examples (basic/complete/secure + feature-specific)

**Cel:** Przyklady zgodne z guide, uruchamialne lokalnie.

**Wymagane:**
- `examples/basic`: minimalny VM + VNet/subnet + NIC + public IP + RDP (haslo).
- `examples/complete`: data disks + boot diagnostics + identity + extensions
  + diag settings + custom_data.
- `examples/secure`: brak public IP, private subnet, NSG z ograniczonym RDP,
...
### TASK-039-8: Testy (unit + integration + negative)

**Cel:** Zgodnosc z `docs/TESTING_GUIDE`.

**Unit tests (tftest.hcl):**
- walidacje admin (username/password).
...
- `computer_name` niezgodny z zasadami Windows.

---

### TASK-039-9: Automatyzacja i release
...
- Modul `modules/azurerm_windows_virtual_machine` spelnia layout z MODULE_GUIDE.
...
- `docs/_CHANGELOG/NNN-YYYY-MM-DD-windows-virtual-machine.md`
