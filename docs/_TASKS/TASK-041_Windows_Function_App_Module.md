# TASK-041: Azure Windows Function App module (full feature scope)
# FileName: TASK-041_Windows_Function_App_Module.md
...
Stworzyc nowy modul `modules/azurerm_windows_function_app` zgodny z:
- `docs/MODULE_GUIDE/`
- `docs/TESTING_GUIDE/`
...
`azurerm_windows_function_app` oraz powiazane sub-resources App Service.
Linux Function App jest osobnym modulem (TASK-036).
...
**Wstepnie zidentyfikowane (azurerm 4.57.0, do potwierdzenia w TASK-041-1):**
- `azurerm_windows_function_app`
- `azurerm_windows_function_app_slot`

**Out-of-scope:**
- `azurerm_linux_function_app` (osobny modul, TASK-036)
...
1) **Atomic scope**  
   Modul zarzadza jednym Windows Function App jako primary resource. Dodatkowe
...
- Runtime: `functions_extension_version` + `application_stack` (Windows)
...
- `site_config`:
  ...
  - `use_32_bit_worker` (jesli wspierane)
...
### TASK-041-1: Discovery / feature inventory

**Cel:** Potwierdzic pelny zakres funkcji provider `azurerm` dla Windows Function App i slotow.
...
### TASK-041-3: Core resource (Windows Function App)
...
- **site_config**: `always_on`, `ftps_state`, `http2_enabled`,
  ...
  `vnet_route_all_enabled`, `application_stack` (Windows),
  `use_32_bit_worker` (jesli wspierane)
- **timeouts** (optional)
...
### TASK-041-4: Slots (Windows)

**Cel:** Pelne wsparcie slotow Windows Function App.
...
### TASK-041-5: Diagnostic settings (inline, AKS pattern)

**Cel:** Wbudowane `azurerm_monitor_diagnostic_setting` dla Windows Function App.
...
### TASK-041-6: Dokumentacja modulu
...
### TASK-041-7: Examples (basic/complete/secure + feature-specific)

**Cel:** Przyklady zgodne z guide, uruchamialne lokalnie.

**Wymagane:**
- `examples/basic`: minimalna konfiguracja Windows.
...
- `examples/zip-deploy`
- `examples/use-32-bit-worker` (jesli wspierane)
...
### TASK-041-8: Testy (unit + integration + negative)
...
### TASK-041-9: Automatyzacja i release
...
- Modul `modules/azurerm_windows_function_app` spelnia layout z MODULE_GUIDE.
...
- `docs/_CHANGELOG/NNN-YYYY-MM-DD-windows-function-app.md`
