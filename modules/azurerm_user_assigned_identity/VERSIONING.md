...
```
UAIv{major}.{minor}.{patch}
```

Where:
- `UAI` = azurerm_user_assigned_identity module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `UAIv1.0.0` - First stable release
- `UAIv1.1.0` - Minor feature addition (from `feat:` commits)
- `UAIv1.0.1` - Bug fix (from `fix:` commits)
- `UAIv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(user-assigned-identity)!: change default behavior
...
```bash
feat(user-assigned-identity): add support for new Azure feature
feat(user-assigned-identity): implement additional configuration options
```
...
```bash
fix(user-assigned-identity): correct validation for resource names
perf(user-assigned-identity): optimize resource creation logic
docs(user-assigned-identity): update README with new examples
```
...
```bash
chore(user-assigned-identity): update .gitignore
test(user-assigned-identity): add unit tests for validation
```
...
- Commits include module scope: `feat(user-assigned-identity): description`
...
module "user_assigned_identity" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azurerm_user_assigned_identity?ref=UAIv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| UAIv1.0.x | >= 1.12.2 | 4.57.0 (pinned) | TBD |
...
```hcl
module "user_assigned_identity" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/azurerm_user_assigned_identity?ref=UAIv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=UAIv1.2.3`)
...
feat(user-assigned-identity): add new functionality
fix(user-assigned-identity): resolve validation issue
...
feat(user-assigned-identity): add support for private endpoints
...
feat(user-assigned-identity)!: restructure input variables
