...
```
PGFSDBv{major}.{minor}.{patch}
```

Where:
- `PGFSDB` = azurerm_postgresql_flexible_server_database module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `PGFSDBv1.0.0` - First stable release
- `PGFSDBv1.1.0` - Minor feature addition (from `feat:` commits)
- `PGFSDBv1.0.1` - Bug fix (from `fix:` commits)
- `PGFSDBv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(postgresql-flexible-server-database)!: change default behavior
...
```bash
feat(postgresql-flexible-server-database): add support for new Azure feature
feat(postgresql-flexible-server-database): implement additional configuration options
```
...
```bash
fix(postgresql-flexible-server-database): correct validation for resource names
perf(postgresql-flexible-server-database): optimize resource creation logic
docs(postgresql-flexible-server-database): update README with new examples
```
...
```bash
chore(postgresql-flexible-server-database): update .gitignore
test(postgresql-flexible-server-database): add unit tests for validation
```
...
- Commits include module scope: `feat(postgresql-flexible-server-database): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `PGFSDBv1.2.0`)
...
module "postgresql_flexible_server_database" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azurerm_postgresql_flexible_server_database?ref=PGFSDBv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| PGFSDBv1.0.x | >= 1.3.0 | 4.36.0 (pinned) | TBD |
...
```hcl
module "postgresql_flexible_server_database" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/azurerm_postgresql_flexible_server_database?ref=PGFSDBv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=PGFSDBv1.2.3`)
...
feat(postgresql-flexible-server-database): add new functionality
fix(postgresql-flexible-server-database): resolve validation issue
...
feat(postgresql-flexible-server-database): add support for private endpoints
...
feat(postgresql-flexible-server-database)!: restructure input variables
