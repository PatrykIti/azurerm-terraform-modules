...
```
BASTIONv{major}.{minor}.{patch}
```

Where:
- `BASTION` = azurerm_bastion_host module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `BASTIONv1.0.0` - First stable release
- `BASTIONv1.1.0` - Minor feature addition (from `feat:` commits)
- `BASTIONv1.0.1` - Bug fix (from `fix:` commits)
- `BASTIONv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(bastion-host)!: change default behavior
...
```bash
feat(bastion-host): add support for new Azure feature
feat(bastion-host): implement additional configuration options
```
...
```bash
fix(bastion-host): correct validation for resource names
perf(bastion-host): optimize resource creation logic
docs(bastion-host): update README with new examples
```
...
```bash
chore(bastion-host): update .gitignore
test(bastion-host): add unit tests for validation
```
...
- Commits include module scope: `feat(bastion-host): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `BASTIONv1.2.0`)
...
module "bastion_host" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azurerm_bastion_host?ref=BASTIONv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| BASTIONv1.0.x | >= 1.12.2 | 4.57.0 (pinned) | TBD |
...
```hcl
module "bastion_host" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/azurerm_bastion_host?ref=BASTIONv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=BASTIONv1.2.3`)
...
feat(bastion-host): add new functionality
fix(bastion-host): resolve validation issue
...
feat(bastion-host): add support for private endpoints
...
feat(bastion-host)!: restructure input variables
