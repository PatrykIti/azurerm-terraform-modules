# Contributing to azuredevops_servicehooks Module
...
6. Commit using conventional commits: `feat(azuredevops-servicehooks): add new feature`
...
```bash
feat(azuredevops-servicehooks): add support for new feature
fix(azuredevops-servicehooks): correct validation logic
docs(azuredevops-servicehooks): update README examples
```
...
2. Run tests locally before submitting PR:
   ```bash
   cd modules/azuredevops_servicehooks/tests
   make test
   ```
