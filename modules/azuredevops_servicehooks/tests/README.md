# Azure DevOps Service Hooks Module Tests

This directory contains automated tests for the Azure DevOps Service Hooks Terraform module using [Terratest](https://terratest.gruntwork.io/).
...
4. **Azure DevOps Project**: Project ID for service hook creation
...
```bash
go test -v -run TestBasicAzuredevopsServicehooks -timeout 30m
```
...
- `azuredevops_servicehooks_test.go` - Basic, complete, secure, and validation tests
...
- `fixtures/basic/` - Basic webhook configuration
- `fixtures/complete/` - Webhook with build filter
- `fixtures/secure/` - Webhook with restricted permissions
...
```bash
go test -v -run TestBasicAzuredevopsServicehooks
```
...
3. **Resource Conflicts**: Ensure unique webhook naming
