...
```
KNSv{major}.{minor}.{patch}
```

Where:
- `KNS` = kubernetes_namespace module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `KNSv1.0.0` - First stable release
- `KNSv1.1.0` - Minor feature addition (from `feat:` commits)
- `KNSv1.0.1` - Bug fix (from `fix:` commits)
- `KNSv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(kubernetes-namespace)!: change default behavior
...
```bash
feat(kubernetes-namespace): add support for new Azure feature
feat(kubernetes-namespace): implement additional configuration options
```
...
```bash
fix(kubernetes-namespace): correct validation for resource names
perf(kubernetes-namespace): optimize resource creation logic
docs(kubernetes-namespace): update README with new examples
```
...
```bash
chore(kubernetes-namespace): update .gitignore
test(kubernetes-namespace): add unit tests for validation
```
...
- Commits include module scope: `feat(kubernetes-namespace): description`
...
module "kubernetes_namespace" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/kubernetes_namespace?ref=KNSv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| KNSv1.0.x | >= 1.3.0 | 4.36.0 (pinned) | TBD |
...
```hcl
module "kubernetes_namespace" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/kubernetes_namespace?ref=KNSv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=KNSv1.2.3`)
...
feat(kubernetes-namespace): add new functionality
fix(kubernetes-namespace): resolve validation issue
...
feat(kubernetes-namespace): add support for private endpoints
...
feat(kubernetes-namespace)!: restructure input variables
