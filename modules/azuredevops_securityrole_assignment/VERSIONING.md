...
```
ADOSRAv{major}.{minor}.{patch}
```

Where:
- `ADOSRA` = azuredevops_securityrole_assignment module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `ADOSRAv1.0.0` - First stable release
- `ADOSRAv1.1.0` - Minor feature addition (from `feat:` commits)
- `ADOSRAv1.0.1` - Bug fix (from `fix:` commits)
- `ADOSRAv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(azuredevops-securityrole-assignment)!: change default behavior
...
```bash
feat(azuredevops-securityrole-assignment): add support for new Azure feature
feat(azuredevops-securityrole-assignment): implement additional configuration options
```
...
```bash
fix(azuredevops-securityrole-assignment): correct validation for resource names
perf(azuredevops-securityrole-assignment): optimize resource creation logic
docs(azuredevops-securityrole-assignment): update README with new examples
```
...
```bash
chore(azuredevops-securityrole-assignment): update .gitignore
test(azuredevops-securityrole-assignment): add unit tests for validation
```
...
- Commits include module scope: `feat(azuredevops-securityrole-assignment): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `ADOSRAv1.2.0`)
...
module "azuredevops_securityrole_assignment" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azuredevops_securityrole_assignment?ref=ADOSRAv1.2.0"
}
```
...
|----------------|-------------------|-----------------------|-------|
| ADOSRAv1.0.x | >= 1.12.2 | 1.12.2 (pinned) | Aligns with module guide baseline |
...
```hcl
module "azuredevops_securityrole_assignment" {
  source = "git::https://github.com/yourusername/azurerm-terraform-modules//modules/azuredevops_securityrole_assignment?ref=ADOSRAv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=ADOSRAv1.2.3`)
...
feat(azuredevops-securityrole-assignment): add new functionality
fix(azuredevops-securityrole-assignment): resolve validation issue
...
feat(azuredevops-securityrole-assignment): add support for private endpoints
...
feat(azuredevops-securityrole-assignment)!: restructure input variables
