...
```
ADOUv{major}.{minor}.{patch}
```

Where:
- `ADOU` = azuredevops_user_entitlement module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `ADOUv1.0.0` - First stable release
- `ADOUv1.1.0` - Minor feature addition (from `feat:` commits)
- `ADOUv1.0.1` - Bug fix (from `fix:` commits)
- `ADOUv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(azuredevops-user-entitlement)!: change default behavior
...
```bash
feat(azuredevops-user-entitlement): add support for new Azure feature
feat(azuredevops-user-entitlement): implement additional configuration options
```
...
```bash
fix(azuredevops-user-entitlement): correct validation for resource names
perf(azuredevops-user-entitlement): optimize resource creation logic
docs(azuredevops-user-entitlement): update README with new examples
```
...
```bash
chore(azuredevops-user-entitlement): update .gitignore
test(azuredevops-user-entitlement): add unit tests for validation
```
...
- Commits include module scope: `feat(azuredevops-user-entitlement): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `ADOUv1.2.0`)
...
module "azuredevops_user_entitlement" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azuredevops_user_entitlement?ref=ADOUv1.2.0"
}
```
...
|----------------|-------------------|-----------------------|-------|
| ADOUv1.0.x | >= 1.12.2 | 1.12.2 (pinned) | Aligns with module guide baseline |
...
```hcl
module "azuredevops_user_entitlement" {
  source = "git::https://github.com/yourusername/azurerm-terraform-modules//modules/azuredevops_user_entitlement?ref=ADOUv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=ADOUv1.2.3`)
...
feat(azuredevops-user-entitlement): add new functionality
fix(azuredevops-user-entitlement): resolve validation issue
...
feat(azuredevops-user-entitlement): add support for private endpoints
...
feat(azuredevops-user-entitlement)!: restructure input variables
