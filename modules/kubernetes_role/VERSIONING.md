...
```
KROLEv{major}.{minor}.{patch}
```

Where:
- `KROLE` = kubernetes_role module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `KROLEv1.0.0` - First stable release
- `KROLEv1.1.0` - Minor feature addition (from `feat:` commits)
- `KROLEv1.0.1` - Bug fix (from `fix:` commits)
- `KROLEv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(kubernetes-role)!: change default behavior
...
```bash
feat(kubernetes-role): add support for new Azure feature
feat(kubernetes-role): implement additional configuration options
```
...
```bash
fix(kubernetes-role): correct validation for resource names
perf(kubernetes-role): optimize resource creation logic
docs(kubernetes-role): update README with new examples
```
...
```bash
chore(kubernetes-role): update .gitignore
test(kubernetes-role): add unit tests for validation
```
...
- Commits include module scope: `feat(kubernetes-role): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `KROLEv1.2.0`)
...
module "kubernetes_role" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/kubernetes_role?ref=KROLEv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| KROLEv1.0.x | >= 1.3.0 | 4.36.0 (pinned) | TBD |
...
```hcl
module "kubernetes_role" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/kubernetes_role?ref=KROLEv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=KROLEv1.2.3`)
...
feat(kubernetes-role): add new functionality
fix(kubernetes-role): resolve validation issue
...
feat(kubernetes-role): add support for private endpoints
...
feat(kubernetes-role)!: restructure input variables
