# Azure Kubernetes Role Module - Initial Release
resource "azurerm_kubernetes_role" "main" {
  name                = var.name
...
resource "azurerm_kubernetes_role_network_rules" "main" {
  count = var.network_rules != null ? 1 : 0

  # TODO: Configure network rules based on resource type
  # storage_account_id = azurerm_kubernetes_role.main.id
...
resource "azurerm_private_endpoint" "main" {
  ...
  private_service_connection {
    ...
    private_connection_resource_id = azurerm_kubernetes_role.main.id
...
resource "azurerm_monitor_diagnostic_setting" "main" {
  ...
  target_resource_id             = azurerm_kubernetes_role.main.id
