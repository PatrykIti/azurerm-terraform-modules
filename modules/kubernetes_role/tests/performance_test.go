...
// BenchmarkKubernetesRoleCreationSimple benchmarks simple kubernetes_role creation
func BenchmarkKubernetesRoleCreationSimple(b *testing.B) {
	b.ReportAllocs()
...
// BenchmarkKubernetesRoleCreationWithFeatures benchmarks kubernetes_role with various features
func BenchmarkKubernetesRoleCreationWithFeatures(b *testing.B) {
	b.ReportAllocs()
...
// BenchmarkKubernetesRoleCreationWithScale benchmarks with different scale configurations
func BenchmarkKubernetesRoleCreationWithScale(b *testing.B) {
	b.ReportAllocs()
...
// BenchmarkKubernetesRoleParallelCreation benchmarks parallel creation of kubernetes_role
func BenchmarkKubernetesRoleParallelCreation(b *testing.B) {
...
// TestKubernetesRoleCreationTime validates creation time is within acceptable limits
func TestKubernetesRoleCreationTime(t *testing.T) {
	...
	// kubernetes_role creation should complete within 5 minutes
	// Adjust this based on the actual expected creation time for the resource
	maxDuration := 5 * time.Minute
	require.LessOrEqual(t, duration, maxDuration,
		"Kubernetes Role creation took %v, expected less than %v", duration, maxDuration)

	t.Logf("Kubernetes Role created in %v", duration)
}

// TestKubernetesRoleScaling tests creating multiple kubernetes_role instances
func TestKubernetesRoleScaling(t *testing.T) {
	if testing.Short() {
	...
	for i := 0; i < instanceCount; i++ {
		...
		t.Logf("Kubernetes Role instance %d created in %v", i, creationTimes[i])
	}
	...
	t.Logf("Average creation time for %d kubernetes_role instances: %v", instanceCount, avgTime)
...
// TestKubernetesRoleUpdatePerformance tests update performance
func TestKubernetesRoleUpdatePerformance(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping performance test in short mode")
	}
	t.Parallel()
	...
	updateScenarios := []struct {
		...
		// Add more update scenarios specific to kubernetes_role
	}
...
// TestKubernetesRoleDestroyPerformance tests destroy performance
func TestKubernetesRoleDestroyPerformance(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping performance test in short mode")
	}
	t.Parallel()
	...
	t.Logf("Kubernetes Role destroyed in %v", destroyTime)
