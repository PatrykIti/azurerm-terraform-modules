...
```
ADOEPv{major}.{minor}.{patch}
```

Where:
- `ADOEP` = azuredevops_elastic_pool module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `ADOEPv1.0.0` - First stable release
- `ADOEPv1.1.0` - Minor feature addition (from `feat:` commits)
- `ADOEPv1.0.1` - Bug fix (from `fix:` commits)
- `ADOEPv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(azuredevops-elastic-pool)!: change default behavior
...
```bash
feat(azuredevops-elastic-pool): add support for new Azure feature
feat(azuredevops-elastic-pool): implement additional configuration options
```
...
```bash
fix(azuredevops-elastic-pool): correct validation for resource names
perf(azuredevops-elastic-pool): optimize resource creation logic
docs(azuredevops-elastic-pool): update README with new examples
```
...
```bash
chore(azuredevops-elastic-pool): update .gitignore
test(azuredevops-elastic-pool): add unit tests for validation
```
...
- Commits include module scope: `feat(azuredevops-elastic-pool): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `ADOEPv1.2.0`)
...
module "azuredevops_elastic_pool" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azuredevops_elastic_pool?ref=ADOEPv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| ADOEPv1.0.x | >= 1.3.0 | 4.36.0 (pinned) | TBD |
...
```hcl
module "azuredevops_elastic_pool" {
  source = "git::https://github.com/yourusername/azurerm-terraform-modules//modules/azuredevops_elastic_pool?ref=ADOEPv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=ADOEPv1.2.3`)
...
feat(azuredevops-elastic-pool): add new functionality
fix(azuredevops-elastic-pool): resolve validation issue
...
feat(azuredevops-elastic-pool): add support for private endpoints
...
feat(azuredevops-elastic-pool)!: restructure input variables
