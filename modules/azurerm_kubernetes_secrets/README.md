...
- **manual**: Caller-provided values → Kubernetes Secret (manual rotation, secrets in state)
...
- **manual** expects secret values to be provided by the caller (Key Vault reads happen outside the module).
...
### Manual (caller-provided values → K8s Secret)
```hcl
...
### Migration (manual strategy)

Manual secrets now accept **values** instead of Key Vault references.
...
<!-- BEGIN_EXAMPLES -->
- [Basic](examples/basic) - This example demonstrates the **manual** strategy with caller-provided values: Key Vault → Terraform → Kubernetes Secret.
...
| <a name="input_manual"></a> [manual](#input\_manual) | Manual strategy configuration (caller-provided values -> Kubernetes Secret).<br/><br/>kubernetes\_secret\_type: Kubernetes Secret type (default: Opaque).<br/>secrets: List of mappings from provided values to Kubernetes Secret keys. | <pre>object({<br/>    kubernetes_secret_type = optional(string, "Opaque")<br/>    secrets = list(object({<br/>      name                  = string<br/>      kubernetes_secret_key = string<br/>      value                 = string<br/>    }))<br/>  })</pre> | `null` | no |
