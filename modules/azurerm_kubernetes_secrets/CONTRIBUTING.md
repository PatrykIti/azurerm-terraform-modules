# Contributing to azurerm_kubernetes_secrets Module
...
6. Commit using conventional commits: `feat(kubernetes-secrets): add new feature`
...
```bash
feat(kubernetes-secrets): add support for new feature
fix(kubernetes-secrets): correct validation logic
docs(kubernetes-secrets): update README examples
```
...
2. Run tests locally before submitting PR:
   ```bash
   cd modules/azurerm_kubernetes_secrets/tests
   make test
   ```
