...
```
ADOWKv{major}.{minor}.{patch}
```

Where:
- `ADOWK` = azuredevops_work_items module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `ADOWKv1.0.0` - First stable release
- `ADOWKv1.1.0` - Minor feature addition (from `feat:` commits)
- `ADOWKv1.0.1` - Bug fix (from `fix:` commits)
- `ADOWKv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(azuredevops-work-items)!: change default behavior
...
```bash
feat(azuredevops-work-items): add support for new Azure feature
feat(azuredevops-work-items): implement additional configuration options
```
...
```bash
fix(azuredevops-work-items): correct validation for resource names
perf(azuredevops-work-items): optimize resource creation logic
docs(azuredevops-work-items): update README with new examples
```
...
```bash
chore(azuredevops-work-items): update .gitignore
test(azuredevops-work-items): add unit tests for validation
```
...
- Commits include module scope: `feat(azuredevops-work-items): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `ADOWKv1.2.0`)
...
module "azuredevops_work_items" {
  source = "github.com/org/repo//modules/azuredevops_work_items?ref=ADOWKv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| ADOWKv1.0.x | >= 1.3.0 | 4.36.0 (pinned) | TBD |
...
- **Production**: Always use specific version tags (e.g., `ref=ADOWKv1.2.3`)
...
feat(azuredevops-work-items): add new functionality
fix(azuredevops-work-items): resolve validation issue
...
feat(azuredevops-work-items): add support for private endpoints
...
feat(azuredevops-work-items)!: restructure input variables
