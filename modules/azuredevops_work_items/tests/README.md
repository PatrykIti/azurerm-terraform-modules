# Azure DevOps Work Items Module Tests

This directory contains automated tests for the Azure DevOps Work Items Terraform module using [Terratest](https://terratest.gruntwork.io/).
...
4. **Azure DevOps Project**: Project ID for work item creation
...
```bash
go test -v -run TestBasicAzuredevopsWorkItems -timeout 30m
```
...
- `azuredevops_work_items_test.go` - Basic, complete, secure, and validation tests
...
```bash
go test -v -run TestBasicAzuredevopsWorkItems
```
...
3. **Resource Conflicts**: Ensure unique work item titles
