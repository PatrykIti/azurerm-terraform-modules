...
| <a name="input_query_folders"></a> [query\_folders](#input\_query\_folders) | List of work item query folders to manage. parent\_key must reference a top-level folder (no parent\_key). | <pre>list(object({<br/>    key        = optional(string)<br/>    project_id = optional(string)<br/>    name       = string<br/>    area       = optional(string)<br/>    parent_id  = optional(number)<br/>    parent_key = optional(string)<br/>  }))</pre> | `[]` | no |
