...
```
AISv{major}.{minor}.{patch}
```

Where:
- `AISv` = azurerm_ai_services module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `AISv1.0.0` - First stable release
- `AISv1.1.0` - Minor feature addition (from `feat:` commits)
- `AISv1.0.1` - Bug fix (from `fix:` commits)
- `AISv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(ai-services)!: change default behavior
...
```bash
feat(ai-services): add support for new Azure feature
feat(ai-services): implement additional configuration options
```
...
```bash
fix(ai-services): correct validation for resource names
perf(ai-services): optimize resource creation logic
docs(ai-services): update README with new examples
```
...
```bash
chore(ai-services): update .gitignore
test(ai-services): add unit tests for validation
```
...
- Commits include module scope: `feat(ai-services): description`
...
module "ai_services" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azurerm_ai_services?ref=AISv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| AISv1.0.x | >= 1.12.2 | 4.57.0 (pinned) | TBD |
...
```hcl
module "ai_services" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/azurerm_ai_services?ref=AISv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=AISv1.2.3`)
...
feat(ai-services): add new functionality
fix(ai-services): resolve validation issue
...
feat(ai-services): add support for private endpoints
...
feat(ai-services)!: restructure input variables
