...
Azure DevOps pipelines module for managing a single build definition (use module-level for_each for multiples) with strict-child permissions and resource authorizations.
...
- [Complete](examples/complete) - This example demonstrates creating multiple YAML pipelines with strict-child resource authorizations.
- [Secure](examples/secure) - This example demonstrates a pipeline with strict-child permissions and explicit resource authorizations.
<!-- END_EXAMPLES -->
...
| <a name="input_build_definition_permissions"></a> [build\_definition\_permissions](#input\_build\_definition\_permissions) | List of build definition permissions to assign to the build definition created by this module. | <pre>list(object({<br/>    key         = optional(string)<br/>    principal   = string<br/>    permissions = map(string)<br/>    replace     = optional(bool, true)<br/>  }))</pre> | `[]` | no |
...
| <a name="input_pipeline_authorizations"></a> [pipeline\_authorizations](#input\_pipeline\_authorizations) | List of resource authorizations to assign to the build definition created by this module. | <pre>list(object({<br/>    key         = optional(string)<br/>    resource_id = string<br/>    type        = string<br/>  }))</pre> | `[]` | no |
