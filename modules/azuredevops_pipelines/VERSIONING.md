...
```bash
feat(azuredevops-pipelines)!: change default behavior
...
```bash
feat(azuredevops-pipelines): add support for new Azure feature
feat(azuredevops-pipelines): implement additional configuration options
```
...
```bash
fix(azuredevops-pipelines): correct validation for resource names
perf(azuredevops-pipelines): optimize resource creation logic
docs(azuredevops-pipelines): update README with new examples
```
...
```bash
chore(azuredevops-pipelines): update .gitignore
test(azuredevops-pipelines): add unit tests for validation
```
...
- Commits include module scope: `feat(azuredevops-pipelines): description`
...
module "azuredevops_pipelines" {
  source = "github.com/org/repo//modules/azuredevops_pipelines?ref=ADORv1.2.0"
}
```
...
feat(azuredevops-pipelines): add new functionality
fix(azuredevops-pipelines): resolve validation issue
...
feat(azuredevops-pipelines): add support for private endpoints
...
feat(azuredevops-pipelines)!: restructure input variables
