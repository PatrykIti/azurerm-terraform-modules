...
```
LAWv{major}.{minor}.{patch}
```

Where:
- `LAW` = azurerm_log_analytics_workspace module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `LAWv1.0.0` - First stable release
- `LAWv1.1.0` - Minor feature addition (from `feat:` commits)
- `LAWv1.0.1` - Bug fix (from `fix:` commits)
- `LAWv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(log-analytics-workspace)!: change default behavior
...
```bash
feat(log-analytics-workspace): add support for new Azure feature
feat(log-analytics-workspace): implement additional configuration options
```
...
```bash
fix(log-analytics-workspace): correct validation for resource names
perf(log-analytics-workspace): optimize resource creation logic
docs(log-analytics-workspace): update README with new examples
```
...
```bash
chore(log-analytics-workspace): update .gitignore
test(log-analytics-workspace): add unit tests for validation
```
...
- Commits include module scope: `feat(log-analytics-workspace): description`
...
module "log_analytics_workspace" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azurerm_log_analytics_workspace?ref=LAWv1.2.0"
}
```
...
```hcl
module "log_analytics_workspace" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/azurerm_log_analytics_workspace?ref=LAWv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=LAWv1.2.3`)
...
feat(log-analytics-workspace): add new functionality
fix(log-analytics-workspace): resolve validation issue
...
feat(log-analytics-workspace): add support for private endpoints
...
feat(log-analytics-workspace)!: restructure input variables
