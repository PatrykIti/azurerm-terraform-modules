...
```
ADOEXv{major}.{minor}.{patch}
```

Where:
- `ADOEX` = azuredevops_extension module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `ADOEXv1.0.0` - First stable release
- `ADOEXv1.1.0` - Minor feature addition (from `feat:` commits)
- `ADOEXv1.0.1` - Bug fix (from `fix:` commits)
- `ADOEXv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(azuredevops-extension)!: change default behavior
...
```bash
feat(azuredevops-extension): add support for new Azure feature
feat(azuredevops-extension): implement additional configuration options
```
...
```bash
fix(azuredevops-extension): correct validation for resource names
perf(azuredevops-extension): optimize resource creation logic
docs(azuredevops-extension): update README with new examples
```
...
```bash
chore(azuredevops-extension): update .gitignore
test(azuredevops-extension): add unit tests for validation
```
...
- Commits include module scope: `feat(azuredevops-extension): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `ADOEXv1.2.0`)
...
module "azuredevops_extension" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azuredevops_extension?ref=ADOEXv1.2.0"
}
```
...
- **Production**: Always use specific version tags (e.g., `ref=ADOEXv1.2.3`)
...
feat(azuredevops-extension): add new functionality
fix(azuredevops-extension): resolve validation issue
...
feat(azuredevops-extension): add support for private endpoints
...
feat(azuredevops-extension)!: restructure input variables
