# Contributing to azuredevops_extension Module
...
6. Commit using conventional commits: `feat(azuredevops-extension): add new feature`
...
```bash
feat(azuredevops-extension): add support for new feature
fix(azuredevops-extension): correct validation logic
docs(azuredevops-extension): update README examples
```
...
2. Run tests locally before submitting PR:
   ```bash
   cd modules/azuredevops_extension/tests
   make test
   ```
