# Contributing to azuredevops_project Module
...
6. Commit using conventional commits: `feat(azuredevops-project): add new feature`
...
```bash
feat(azuredevops-project): add support for new feature
fix(azuredevops-project): correct validation logic
docs(azuredevops-project): update README examples
```
...
2. Run tests locally before submitting PR:
   ```bash
   cd modules/azuredevops_project/tests
   make test
   ```
