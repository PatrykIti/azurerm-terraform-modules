...
```
ADOPv{major}.{minor}.{patch}
```

Where:
- `ADOP` = azuredevops_project module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `ADOPv1.0.0` - First stable release
- `ADOPv1.1.0` - Minor feature addition (from `feat:` commits)
- `ADOPv1.0.1` - Bug fix (from `fix:` commits)
- `ADOPv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(azuredevops-project)!: change default behavior
...
```bash
feat(azuredevops-project): add support for new Azure feature
feat(azuredevops-project): implement additional configuration options
```
...
```bash
fix(azuredevops-project): correct validation for resource names
perf(azuredevops-project): optimize resource creation logic
docs(azuredevops-project): update README with new examples
```
...
```bash
chore(azuredevops-project): update .gitignore
test(azuredevops-project): add unit tests for validation
```
...
- Commits include module scope: `feat(azuredevops-project): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `ADOPv1.2.0`)
...
- **Production**: Always use specific version tags (e.g., `ref=ADOPv1.2.3`)
...
feat(azuredevops-project): add new functionality
fix(azuredevops-project): resolve validation issue
...
feat(azuredevops-project): add support for private endpoints
...
feat(azuredevops-project)!: restructure input variables
