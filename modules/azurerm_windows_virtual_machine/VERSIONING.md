...
```
WINDOWSVMv{major}.{minor}.{patch}
```

Where:
- `WINDOWSVM` = azurerm_windows_virtual_machine module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `WINDOWSVMv1.0.0` - First stable release
- `WINDOWSVMv1.1.0` - Minor feature addition (from `feat:` commits)
- `WINDOWSVMv1.0.1` - Bug fix (from `fix:` commits)
- `WINDOWSVMv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(windows-virtual-machine)!: change default behavior
...
```bash
feat(windows-virtual-machine): add support for new Azure feature
feat(windows-virtual-machine): implement additional configuration options
```
...
```bash
fix(windows-virtual-machine): correct validation for resource names
perf(windows-virtual-machine): optimize resource creation logic
docs(windows-virtual-machine): update README with new examples
```
...
```bash
chore(windows-virtual-machine): update .gitignore
test(windows-virtual-machine): add unit tests for validation
```
...
- Commits include module scope: `feat(windows-virtual-machine): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `WINDOWSVMv1.2.0`)
...
module "windows_virtual_machine" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azurerm_windows_virtual_machine?ref=WINDOWSVMv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| WINDOWSVMv1.0.x | >= 1.12.2 | 4.57.0 (pinned) | TBD |
...
```hcl
module "windows_virtual_machine" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/azurerm_windows_virtual_machine?ref=WINDOWSVMv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=WINDOWSVMv1.2.3`)
...
feat(windows-virtual-machine): add new functionality
fix(windows-virtual-machine): resolve validation issue
...
feat(windows-virtual-machine): add support for private endpoints
...
feat(windows-virtual-machine)!: restructure input variables
