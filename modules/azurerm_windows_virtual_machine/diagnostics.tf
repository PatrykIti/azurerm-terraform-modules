data "azurerm_monitor_diagnostic_categories" "windows_virtual_machine" {
  count       = length(var.diagnostic_settings) > 0 ? 1 : 0
  resource_id = azurerm_windows_virtual_machine.windows_virtual_machine.id
}

locals {
  diagnostic_log_categories    = try(data.azurerm_monitor_diagnostic_categories.windows_virtual_machine[0].log_category_types, [])
  diagnostic_log_groups        = try(data.azurerm_monitor_diagnostic_categories.windows_virtual_machine[0].log_category_groups, [])
  diagnostic_metric_categories = try(data.azurerm_monitor_diagnostic_categories.windows_virtual_machine[0].metric_category_types, [])
