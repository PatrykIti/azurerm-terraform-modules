...
```
RDv{major}.{minor}.{patch}
```

Where:
- `RD` = azurerm_role_definition module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `RDv1.0.0` - First stable release
- `RDv1.1.0` - Minor feature addition (from `feat:` commits)
- `RDv1.0.1` - Bug fix (from `fix:` commits)
- `RDv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(role-definition)!: change default behavior
...
```bash
feat(role-definition): add support for new Azure feature
feat(role-definition): implement additional configuration options
```
...
```bash
fix(role-definition): correct validation for resource names
perf(role-definition): optimize resource creation logic
docs(role-definition): update README with new examples
```
...
```bash
chore(role-definition): update .gitignore
test(role-definition): add unit tests for validation
```
...
- Commits include module scope: `feat(role-definition): description`
...
module "role_definition" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azurerm_role_definition?ref=RDv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| RDv1.0.x | >= 1.12.2 | 4.57.0 (pinned) | TBD |
...
```hcl
module "role_definition" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/azurerm_role_definition?ref=RDv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=RDv1.2.3`)
...
feat(role-definition): add new functionality
fix(role-definition): resolve validation issue
...
feat(role-definition): add support for private endpoints
...
feat(role-definition)!: restructure input variables
