...
```
WFUNCv{major}.{minor}.{patch}
```

Where:
- `WFUNC` = azurerm_windows_function_app module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `WFUNCv1.0.0` - First stable release
- `WFUNCv1.1.0` - Minor feature addition (from `feat:` commits)
- `WFUNCv1.0.1` - Bug fix (from `fix:` commits)
- `WFUNCv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(windows-function-app)!: change default behavior
...
```bash
feat(windows-function-app): add support for new Azure feature
feat(windows-function-app): implement additional configuration options
```
...
```bash
fix(windows-function-app): correct validation for resource names
perf(windows-function-app): optimize resource creation logic
docs(windows-function-app): update README with new examples
```
...
```bash
chore(windows-function-app): update .gitignore
test(windows-function-app): add unit tests for validation
```
...
- Commits include module scope: `feat(windows-function-app): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `WFUNCv1.2.0`)
...
module "windows_function_app" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azurerm_windows_function_app?ref=WFUNCv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| WFUNCv1.0.x | >= 1.3.0 | 4.36.0 (pinned) | TBD |
...
```hcl
module "windows_function_app" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/azurerm_windows_function_app?ref=WFUNCv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=WFUNCv1.2.3`)
...
feat(windows-function-app): add new functionality
fix(windows-function-app): resolve validation issue
...
feat(windows-function-app): add support for private endpoints
...
feat(windows-function-app)!: restructure input variables
