...
- FTP basic auth disabled (`ftp_publish_basic_authentication_enabled = false`)
- WebDeploy basic auth disabled (`webdeploy_publish_basic_authentication_enabled = false`)

## Supported Security Controls

- **TLS hardening**: `site_config.minimum_tls_version` and `site_config.scm_minimum_tls_version`
- **Client certificates**: `client_certificate_enabled` and `client_certificate_mode`
...
- **Storage access**: `storage_uses_managed_identity` or `storage_key_vault_secret_id`
...
- `storage_key_vault_secret_id` without a version uses the latest secret version, but propagation can take up to 24 hours after rotation.
...
- [ ] Keep `public_network_access_enabled = false` unless explicitly required.
