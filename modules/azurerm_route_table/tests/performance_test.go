...
// BenchmarkRoute TableCreationSimple benchmarks simple route_table creation
func BenchmarkRoute TableCreationSimple(b *testing.B) {
	b.ReportAllocs()

	for i := 0; i < b.N; i++ {
		b.StopTimer()
		testFolder := test_structure.CopyTerraformFolderToTemp(b, "../..", "azurerm_route_table/tests/fixtures/simple")
...
// BenchmarkRoute TableCreationWithFeatures benchmarks route_table with various features
func BenchmarkRoute TableCreationWithFeatures(b *testing.B) {
	b.ReportAllocs()
...
// BenchmarkRoute TableCreationWithScale benchmarks with different scale configurations
func BenchmarkRoute TableCreationWithScale(b *testing.B) {
	b.ReportAllocs()
...
// BenchmarkRoute TableParallelCreation benchmarks parallel creation of route_table
func BenchmarkRoute TableParallelCreation(b *testing.B) {
...
// TestRoute TableCreationTime validates creation time is within acceptable limits
func TestRoute TableCreationTime(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping performance test in short mode")
	}
	t.Parallel()

	testFolder := test_structure.CopyTerraformFolderToTemp(t, "../..", "azurerm_route_table/tests/fixtures/simple")
	...
	// route_table creation should complete within 5 minutes
	// Adjust this based on the actual expected creation time for the resource
	maxDuration := 5 * time.Minute
	require.LessOrEqual(t, duration, maxDuration,
		"Route Table creation took %v, expected less than %v", duration, maxDuration)

	t.Logf("Route Table created in %v", duration)
}

// TestRoute TableScaling tests creating multiple route_table instances
func TestRoute TableScaling(t *testing.T) {
	if testing.Short() {
	...
	for i := 0; i < instanceCount; i++ {
		...
		t.Logf("Route Table instance %d created in %v", i, creationTimes[i])
	}
	...
	t.Logf("Average creation time for %d route_table instances: %v", instanceCount, avgTime)
...
// TestRoute TableUpdatePerformance tests update performance
func TestRoute TableUpdatePerformance(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping performance test in short mode")
	}
	t.Parallel()
	...
	updateScenarios := []struct {
		...
		// Add more update scenarios specific to route_table
	}
...
// TestRoute TableDestroyPerformance tests destroy performance
func TestRoute TableDestroyPerformance(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping performance test in short mode")
	}
	t.Parallel()
	...
	t.Logf("Route Table destroyed in %v", destroyTime)
