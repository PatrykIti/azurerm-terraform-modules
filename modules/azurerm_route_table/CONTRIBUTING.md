# Contributing to azurerm_route_table Module
...
6. Commit using conventional commits: `feat(route-table): add new feature`
...
```bash
feat(route-table): add support for new feature
fix(route-table): correct validation logic
docs(route-table): update README examples
```
...
2. Run tests locally before submitting PR:
   ```bash
   cd modules/azurerm_route_table/tests
   make test
   ```
