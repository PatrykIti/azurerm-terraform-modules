# Secure fixture for kubernetes role binding
...
resource "azurerm_resource_group" "test" {
  name     = "rg-krb-sec-${var.random_suffix}"
  location = var.location
}

resource "azurerm_virtual_network" "test" {
  name                = "vnet-krb-sec-${var.random_suffix}"
...
resource "azurerm_subnet" "test" {
  name                 = "snet-krb-sec-${var.random_suffix}"
  resource_group_name  = azurerm_resource_group.test.name
...
module "kubernetes_cluster" {
  ...
  name                = "krb-sec-${var.random_suffix}"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  dns_config = {
    dns_prefix = "krb-sec-${var.random_suffix}"
  }
  identity = {
    type = "SystemAssigned"
  }
