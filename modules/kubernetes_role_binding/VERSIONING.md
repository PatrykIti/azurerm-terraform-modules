...
```
KRBv{major}.{minor}.{patch}
```

Where:
- `KRB` = kubernetes_role_binding module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `KRBv1.0.0` - First stable release
- `KRBv1.1.0` - Minor feature addition (from `feat:` commits)
- `KRBv1.0.1` - Bug fix (from `fix:` commits)
- `KRBv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(kubernetes-role-binding)!: change default behavior
...
```bash
feat(kubernetes-role-binding): add support for new Azure feature
feat(kubernetes-role-binding): implement additional configuration options
```
...
```bash
fix(kubernetes-role-binding): correct validation for resource names
perf(kubernetes-role-binding): optimize resource creation logic
docs(kubernetes-role-binding): update README with new examples
```
...
```bash
chore(kubernetes-role-binding): update .gitignore
test(kubernetes-role-binding): add unit tests for validation
```
...
- Commits include module scope: `feat(kubernetes-role-binding): description`
...
module "kubernetes_role_binding" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/kubernetes_role_binding?ref=KRBv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| KRBv1.0.x | >= 1.3.0 | 4.36.0 (pinned) | TBD |
...
```hcl
module "kubernetes_role_binding" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/kubernetes_role_binding?ref=KRBv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=KRBv1.2.3`)
...
feat(kubernetes-role-binding): add new functionality
fix(kubernetes-role-binding): resolve validation issue
...
feat(kubernetes-role-binding): add support for private endpoints
...
feat(kubernetes-role-binding)!: restructure input variables
