...
func TestVirtualNetworkFlowLogs(t *testing.T) {
	...
	testFolder := test_structure.CopyTerraformFolderToTemp(t, "..", "tests/fixtures/flow_logs")
...
func TestVirtualNetworkValidationRules(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			fixturePath:   "tests/fixtures/negative/invalid_name_short",
			expectError:   true,
			errorContains: "Virtual Network name must be 2-80 characters long",
		},
		{
			name:          "InvalidNameLong",
			fixturePath:   "tests/fixtures/negative/invalid_name_long",
			expectError:   true,
			errorContains: "Virtual Network name must be 2-80 characters long",
		},
		{
			name:          "InvalidNameChars",
			fixturePath:   "tests/fixtures/negative/invalid_name_chars",
			expectError:   true,
			errorContains: "Virtual Network name must be 2-80 characters long",
		},
		{
			name:          "EmptyAddressSpace",
			fixturePath:   "tests/fixtures/negative/empty_address_space",
			expectError:   true,
