...
### Examples:
- `PDNSZv1.0.0` - First stable release
- `PDNSZv1.1.0` - Minor feature addition (from `feat:` commits)
- `PDNSZv1.0.1` - Bug fix (from `fix:` commits)
- `PDNSZv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(private-dns-zone)!: change default behavior
...
```bash
feat(private-dns-zone): add support for new Azure feature
feat(private-dns-zone): implement additional configuration options
```
...
```bash
fix(private-dns-zone): correct validation for resource names
perf(private-dns-zone): optimize resource creation logic
docs(private-dns-zone): update README with new examples
```
...
```bash
chore(private-dns-zone): update .gitignore
test(private-dns-zone): add unit tests for validation
```
...
- Commits include module scope: `feat(private-dns-zone): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `PDNSZv1.2.0`)
...
module "private_dns_zone" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azurerm_private_dns_zone?ref=PDNSZv1.2.0"
}
```
...
```hcl
module "private_dns_zone" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/azurerm_private_dns_zone?ref=PDNSZv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=PDNSZv1.2.3`)
...
feat(private-dns-zone): add new functionality
fix(private-dns-zone): resolve validation issue
...
feat(private-dns-zone): add support for private endpoints
...
feat(private-dns-zone)!: restructure input variables
