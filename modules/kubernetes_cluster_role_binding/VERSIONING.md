...
```
KCRBv{major}.{minor}.{patch}
```

Where:
- `KCRB` = kubernetes_cluster_role_binding module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `KCRBv1.0.0` - First stable release
- `KCRBv1.1.0` - Minor feature addition (from `feat:` commits)
- `KCRBv1.0.1` - Bug fix (from `fix:` commits)
- `KCRBv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(kubernetes-cluster-role-binding)!: change default behavior
...
```bash
feat(kubernetes-cluster-role-binding): add support for new Azure feature
feat(kubernetes-cluster-role-binding): implement additional configuration options
```
...
```bash
fix(kubernetes-cluster-role-binding): correct validation for resource names
perf(kubernetes-cluster-role-binding): optimize resource creation logic
docs(kubernetes-cluster-role-binding): update README with new examples
```
...
```bash
chore(kubernetes-cluster-role-binding): update .gitignore
test(kubernetes-cluster-role-binding): add unit tests for validation
```
...
- Commits include module scope: `feat(kubernetes-cluster-role-binding): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `KCRBv1.2.0`)
...
module "kubernetes_cluster_role_binding" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/kubernetes_cluster_role_binding?ref=KCRBv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| KCRBv1.0.x | >= 1.3.0 | 4.36.0 (pinned) | TBD |
...
```hcl
module "kubernetes_cluster_role_binding" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/kubernetes_cluster_role_binding?ref=KCRBv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=KCRBv1.2.3`)
...
feat(kubernetes-cluster-role-binding): add new functionality
fix(kubernetes-cluster-role-binding): resolve validation issue
...
feat(kubernetes-cluster-role-binding): add support for private endpoints
...
feat(kubernetes-cluster-role-binding)!: restructure input variables
