# Complete fixture for kubernetes cluster role binding
terraform {
...
resource "azurerm_resource_group" "test" {
  name     = "rg-kcrb-com-${var.random_suffix}"
  location = var.location
}

resource "azurerm_virtual_network" "test" {
  name                = "vnet-kcrb-com-${var.random_suffix}"
...
resource "azurerm_subnet" "test" {
  name                 = "snet-kcrb-com-${var.random_suffix}"
  resource_group_name  = azurerm_resource_group.test.name
...
module "kubernetes_cluster" {
  ...
  name                = "kcrb-com-${var.random_suffix}"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  dns_config = {
    dns_prefix = "kcrb-com-${var.random_suffix}"
  }
...
module "kubernetes_cluster_role_binding" {
  ...
  subjects = [
    { kind = "User", name = "00000000-0000-0000-0000-000000000001" },
    { kind = "User", name = "00000000-0000-0000-0000-000000000002" }
  ]
