# Contributing to azuredevops_wiki Module
...
6. Commit using conventional commits: `feat(azuredevops-wiki): add new feature`
...
```bash
feat(azuredevops-wiki): add support for new feature
fix(azuredevops-wiki): correct validation logic
docs(azuredevops-wiki): update README examples
```
...
2. Run tests locally before submitting PR:
   ```bash
   cd modules/azuredevops_wiki/tests
   make test
   ```
