...
```
ADOWIv{major}.{minor}.{patch}
```

Where:
- `ADOWI` = azuredevops_wiki module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `ADOWIv1.0.0` - First stable release
- `ADOWIv1.1.0` - Minor feature addition (from `feat:` commits)
- `ADOWIv1.0.1` - Bug fix (from `fix:` commits)
- `ADOWIv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(azuredevops-wiki)!: change default behavior
...
```bash
feat(azuredevops-wiki): add support for new Azure feature
feat(azuredevops-wiki): implement additional configuration options
```
...
```bash
fix(azuredevops-wiki): correct validation for resource names
perf(azuredevops-wiki): optimize resource creation logic
docs(azuredevops-wiki): update README with new examples
```
...
```bash
chore(azuredevops-wiki): update .gitignore
test(azuredevops-wiki): add unit tests for validation
```
...
- Commits include module scope: `feat(azuredevops-wiki): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `ADOWIv1.2.0`)
...
module "azuredevops_wiki" {
  source = "github.com/org/repo//modules/azuredevops_wiki?ref=ADOWIv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| ADOWIv1.0.x | >= 1.3.0 | 4.36.0 (pinned) | TBD |
...
- **Production**: Always use specific version tags (e.g., `ref=ADOWIv1.2.3`)
...
feat(azuredevops-wiki): add new functionality
fix(azuredevops-wiki): resolve validation issue
...
feat(azuredevops-wiki): add support for private endpoints
...
feat(azuredevops-wiki)!: restructure input variables
