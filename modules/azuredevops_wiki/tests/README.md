# Azure DevOps Wiki Module Tests

This directory contains automated tests for the Azure DevOps Wiki Terraform module using [Terratest](https://terratest.gruntwork.io/).
...
4. **Azure DevOps Project**: Project ID for wiki creation
...
```bash
go test -v -run TestBasicAzuredevopsWiki -timeout 30m
```
...
- `azuredevops_wiki_test.go` - Basic, complete, secure, and validation tests
...
```bash
go test -v -run TestBasicAzuredevopsWiki
```
...
3. **Resource Conflicts**: Ensure unique wiki naming
