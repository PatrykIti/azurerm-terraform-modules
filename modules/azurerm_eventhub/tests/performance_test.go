...
// BenchmarkEventhubCreationSimple benchmarks simple eventhub creation
func BenchmarkEventhubCreationSimple(b *testing.B) {
	b.ReportAllocs()
...
// BenchmarkEventhubCreationWithFeatures benchmarks eventhub with various features
func BenchmarkEventhubCreationWithFeatures(b *testing.B) {
	b.ReportAllocs()
...
// BenchmarkEventhubCreationWithScale benchmarks with different scale configurations
func BenchmarkEventhubCreationWithScale(b *testing.B) {
	b.ReportAllocs()
...
// BenchmarkEventhubParallelCreation benchmarks parallel creation of eventhub
func BenchmarkEventhubParallelCreation(b *testing.B) {
...
// TestEventhubCreationTime validates creation time is within acceptable limits
func TestEventhubCreationTime(t *testing.T) {
	if testing.Short() {
	...
	// eventhub creation should complete within 5 minutes
	// Adjust this based on the actual expected creation time for the resource
	maxDuration := 5 * time.Minute
	require.LessOrEqual(t, duration, maxDuration,
		"Event Hub creation took %v, expected less than %v", duration, maxDuration)

	t.Logf("Event Hub created in %v", duration)
}

// TestEventhubScaling tests creating multiple eventhub instances
func TestEventhubScaling(t *testing.T) {
	if testing.Short() {
	...
	for i := 0; i < instanceCount; i++ {
		...
		t.Logf("Event Hub instance %d created in %v", i, creationTimes[i])
	}
	...
	t.Logf("Average creation time for %d eventhub instances: %v", instanceCount, avgTime)
...
// TestEventhubUpdatePerformance tests update performance
func TestEventhubUpdatePerformance(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping performance test in short mode")
	}
	t.Parallel()
...
// TestEventhubDestroyPerformance tests destroy performance
func TestEventhubDestroyPerformance(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping performance test in short mode")
	}
	t.Parallel()
	...
	t.Logf("Event Hub destroyed in %v", destroyTime)
