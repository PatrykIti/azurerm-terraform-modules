...
// Test basic eventhub creation
func TestBasicEventhub(t *testing.T) {
	t.Parallel()
	...
	test_structure.RunTestStage(t, "validate", func() {
		...
		// Add eventhub specific validations here
	})
}

// Test complete eventhub with all features
...
func TestCaptureEventhub(t *testing.T) {
	...
	testFolder := test_structure.CopyTerraformFolderToTemp(t, ".", "fixtures/capture")
	defer test_structure.RunTestStage(t, "cleanup", func() {
		terraform.Destroy(t, getTerraformOptions(t, testFolder))
	})
...
func TestConsumerGroupsEventhub(t *testing.T) {
	...
	testFolder := test_structure.CopyTerraformFolderToTemp(t, ".", "fixtures/consumer_groups")
	defer test_structure.RunTestStage(t, "cleanup", func() {
		terraform.Destroy(t, getTerraformOptions(t, testFolder))
	})
	...
	test_structure.RunTestStage(t, "validate", func() {
		...
		// Validate consumer groups example
		assert.NotEmpty(t, resourceID)
	})
}

// Negative test cases for validation rules
func TestEventhubValidationRules(t *testing.T) {
	t.Parallel()
	...
	testCases := []struct {
		...
		// Add more validation test cases specific to eventhub
	}
...
func BenchmarkEventhubCreation(b *testing.B) {
