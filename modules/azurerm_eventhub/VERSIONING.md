...
```
EHv{major}.{minor}.{patch}
```

Where:
- `EHv` = azurerm_eventhub module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `EHv1.0.0` - First stable release
- `EHv1.1.0` - Minor feature addition (from `feat:` commits)
- `EHv1.0.1` - Bug fix (from `fix:` commits)
- `EHv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(eventhub)!: change default behavior
...
```bash
feat(eventhub): add support for new Azure feature
feat(eventhub): implement additional configuration options
```
...
```bash
fix(eventhub): correct validation for resource names
perf(eventhub): optimize resource creation logic
docs(eventhub): update README with new examples
```
...
```bash
chore(eventhub): update .gitignore
test(eventhub): add unit tests for validation
```
...
- Commits include module scope: `feat(eventhub): description`
...
module "eventhub" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azurerm_eventhub?ref=EHv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| EHv1.0.x | >= 1.3.0 | 4.36.0 (pinned) | TBD |
...
```hcl
module "eventhub" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/azurerm_eventhub?ref=EHv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=EHv1.2.3`)
...
feat(eventhub): add new functionality
fix(eventhub): resolve validation issue
...
feat(eventhub): add support for private endpoints
...
feat(eventhub)!: restructure input variables
