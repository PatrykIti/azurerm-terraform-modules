# Contributing to azuredevops_identity Module
...
6. Commit using conventional commits: `feat(azuredevops-identity): add new feature`
...
```bash
feat(azuredevops-identity): add support for new feature
fix(azuredevops-identity): correct validation logic
docs(azuredevops-identity): update README examples
```
...
2. Run tests locally before submitting PR:
   ```bash
   cd modules/azuredevops_identity/tests
   make test
   ```
