...
```
ADOIv{major}.{minor}.{patch}
```

Where:
- `ADOI` = azuredevops_identity module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `ADOIv1.0.0` - First stable release
- `ADOIv1.1.0` - Minor feature addition (from `feat:` commits)
- `ADOIv1.0.1` - Bug fix (from `fix:` commits)
- `ADOIv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(azuredevops-identity)!: change default behavior
...
```bash
feat(azuredevops-identity): add support for new Azure feature
feat(azuredevops-identity): implement additional configuration options
```
...
```bash
fix(azuredevops-identity): correct validation for resource names
perf(azuredevops-identity): optimize resource creation logic
docs(azuredevops-identity): update README with new examples
```
...
```bash
chore(azuredevops-identity): update .gitignore
test(azuredevops-identity): add unit tests for validation
```
...
- Commits include module scope: `feat(azuredevops-identity): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `ADOIv1.2.0`)
...
|----------------|-------------------|------------------|-------------------|
| ADOIv1.0.x | >= 1.3.0 | 4.36.0 (pinned) | TBD |
...
- **Production**: Always use specific version tags (e.g., `ref=ADOIv1.2.3`)
...
feat(azuredevops-identity): add new functionality
fix(azuredevops-identity): resolve validation issue
...
feat(azuredevops-identity): add support for private endpoints
...
feat(azuredevops-identity)!: restructure input variables
