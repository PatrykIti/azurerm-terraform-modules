...
```
AISAv{major}.{minor}.{patch}
```

Where:
- `AISAv` = azurerm_ai_services_account module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `AISAv1.0.0` - First stable release
- `AISAv1.1.0` - Minor feature addition (from `feat:` commits)
- `AISAv1.0.1` - Bug fix (from `fix:` commits)
- `AISAv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(ai-services-account)!: change default behavior
...
```bash
feat(ai-services-account): add support for new Azure feature
feat(ai-services-account): implement additional configuration options
```
...
```bash
fix(ai-services-account): correct validation for resource names
perf(ai-services-account): optimize resource creation logic
docs(ai-services-account): update README with new examples
```
...
```bash
chore(ai-services-account): update .gitignore
test(ai-services-account): add unit tests for validation
```
...
- Commits include module scope: `feat(ai-services-account): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `AISAv1.2.0`)
...
module "ai_services_account" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azurerm_ai_services_account?ref=AISAv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| AISAv1.0.x | >= 1.12.2 | 4.57.0 (pinned) | TBD |
...
```hcl
module "ai_services_account" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/azurerm_ai_services_account?ref=AISAv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=AISAv1.2.3`)
...
feat(ai-services-account): add new functionality
fix(ai-services-account): resolve validation issue
...
feat(ai-services-account): add support for private endpoints
...
feat(ai-services-account)!: restructure input variables
