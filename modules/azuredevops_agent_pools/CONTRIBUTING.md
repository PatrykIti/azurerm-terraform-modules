# Contributing to azuredevops_agent_pools Module
...
6. Commit using conventional commits: `feat(azuredevops-agent-pools): add new feature`
...
```bash
feat(azuredevops-agent-pools): add support for new feature
fix(azuredevops-agent-pools): correct validation logic
docs(azuredevops-agent-pools): update README examples
```
...
2. Run tests locally before submitting PR:
   ```bash
   cd modules/azuredevops_agent_pools/tests
   make test
   ```
