...
```
ADOAPv{major}.{minor}.{patch}
```

Where:
- `ADOAP` = azuredevops_agent_pools module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `ADOAPv1.0.0` - First stable release
- `ADOAPv1.1.0` - Minor feature addition (from `feat:` commits)
- `ADOAPv1.0.1` - Bug fix (from `fix:` commits)
- `ADOAPv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(azuredevops-agent-pools)!: change default behavior
...
```bash
feat(azuredevops-agent-pools): add support for new Azure feature
feat(azuredevops-agent-pools): implement additional configuration options
```
...
```bash
fix(azuredevops-agent-pools): correct validation for resource names
perf(azuredevops-agent-pools): optimize resource creation logic
docs(azuredevops-agent-pools): update README with new examples
```
...
```bash
chore(azuredevops-agent-pools): update .gitignore
test(azuredevops-agent-pools): add unit tests for validation
```
...
- Commits include module scope: `feat(azuredevops-agent-pools): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `ADOAPv1.2.0`)
...
module "azuredevops_agent_pools" {
  source = "github.com/org/repo//modules/azuredevops_agent_pools?ref=ADOAPv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| ADOAPv1.0.x | >= 1.3.0 | 4.36.0 (pinned) | TBD |
...
```hcl
module "azuredevops_agent_pools" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/azuredevops_agent_pools?ref=ADOAPv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=ADOAPv1.2.3`)
...
feat(azuredevops-agent-pools): add new functionality
fix(azuredevops-agent-pools): resolve validation issue
...
feat(azuredevops-agent-pools): add support for private endpoints
...
feat(azuredevops-agent-pools)!: restructure input variables
