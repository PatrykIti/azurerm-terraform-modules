variable "resource_group_name" {
  ...
  type        = string
  default     = "rg-pgfs-secure-example"
}
...
variable "virtual_network_name" {
  ...
  type        = string
  default     = "vnet-pgfs-secure-example"
}

variable "subnet_name" {
  description = "Delegated subnet name for the PostgreSQL Flexible Server."
  type        = string
  default     = "snet-pgfs-secure-example"
}

variable "user_assigned_identity_name" {
  description = "User-assigned identity name for CMK access."
  type        = string
  default     = "uai-pgfs-secure-example"
}

variable "key_vault_name" {
  description = "Key Vault name for customer-managed keys (must be globally unique)."
  type        = string
  default     = "kvpgfssecureex001"
}
