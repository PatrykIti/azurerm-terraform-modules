...
```
PGFSv{major}.{minor}.{patch}
```

Where:
- `PGFS` = azurerm_postgresql_flexible_server module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `PGFSv1.0.0` - First stable release
- `PGFSv1.1.0` - Minor feature addition (from `feat:` commits)
- `PGFSv1.0.1` - Bug fix (from `fix:` commits)
- `PGFSv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(postgresql-flexible-server)!: change default behavior
...
```bash
feat(postgresql-flexible-server): add support for new Azure feature
feat(postgresql-flexible-server): implement additional configuration options
```
...
```bash
fix(postgresql-flexible-server): correct validation for resource names
perf(postgresql-flexible-server): optimize resource creation logic
docs(postgresql-flexible-server): update README with new examples
```
...
```bash
chore(postgresql-flexible-server): update .gitignore
test(postgresql-flexible-server): add unit tests for validation
```
...
- Commits include module scope: `feat(postgresql-flexible-server): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `PGFSv1.2.0`)
...
module "postgresql_flexible_server" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azurerm_postgresql_flexible_server?ref=PGFSv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| PGFSv1.0.x | >= 1.3.0 | 4.36.0 (pinned) | TBD |
...
```hcl
module "postgresql_flexible_server" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/azurerm_postgresql_flexible_server?ref=PGFSv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=PGFSv1.2.3`)
...
feat(postgresql-flexible-server): add new functionality
fix(postgresql-flexible-server): resolve validation issue
...
feat(postgresql-flexible-server): add support for private endpoints
...
feat(postgresql-flexible-server)!: restructure input variables
