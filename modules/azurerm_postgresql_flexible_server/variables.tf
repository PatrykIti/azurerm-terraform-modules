...
variable "name" {
  description = "The name of the postgresql_flexible_server. Must be globally unique."
  type        = string

  validation {
    condition     = can(regex("^[a-z0-9]{3,24}$", var.name))
    error_message = "PostgreSQL Flexible Server name must be between 3 and 24 characters long and use numbers and lower-case letters only."
  }
}

variable "resource_group_name" {
  description = "The name of the resource group in which to create the postgresql_flexible_server."
  type        = string
}
...
variable "security_settings" {
  description = "Security configuration for the postgresql_flexible_server."
  type = object({
...
variable "network_rules" {
  description = "Network rules configuration for the postgresql_flexible_server."
  type = object({
...
variable "private_endpoints" {
  description = "List of private endpoints to create for the postgresql_flexible_server."
  type = list(object({
    name                            = string
    subnet_id                       = string
    subresource_names               = optional(list(string), ["postgresql_flexible_server"])
