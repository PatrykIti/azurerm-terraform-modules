...
```
KVv{major}.{minor}.{patch}
```

Where:
- `KV` = azurerm_key_vault module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `KVv1.0.0` - First stable release
- `KVv1.1.0` - Minor feature addition (from `feat:` commits)
- `KVv1.0.1` - Bug fix (from `fix:` commits)
- `KVv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(key-vault)!: change default behavior
...
```bash
feat(key-vault): add support for new Azure feature
feat(key-vault): implement additional configuration options
```
...
```bash
fix(key-vault): correct validation for resource names
perf(key-vault): optimize resource creation logic
docs(key-vault): update README with new examples
```
...
```bash
chore(key-vault): update .gitignore
test(key-vault): add unit tests for validation
```
...
- Commits include module scope: `feat(key-vault): description`
...
module "key_vault" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azurerm_key_vault?ref=KVv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| KVv1.0.x | >= 1.12.2 | 4.57.0 (pinned) | TBD |
...
```hcl
module "key_vault" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/azurerm_key_vault?ref=KVv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=KVv1.2.3`)
...
feat(key-vault): add new functionality
fix(key-vault): resolve validation issue
...
feat(key-vault): add support for private endpoints
...
feat(key-vault)!: restructure input variables
