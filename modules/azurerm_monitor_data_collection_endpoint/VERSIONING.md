...
```
DCEv{major}.{minor}.{patch}
```

Where:
- `DCE` = azurerm_monitor_data_collection_endpoint module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `DCEv1.0.0` - First stable release
- `DCEv1.1.0` - Minor feature addition (from `feat:` commits)
- `DCEv1.0.1` - Bug fix (from `fix:` commits)
- `DCEv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(monitor-dce)!: change default behavior
...
```bash
feat(monitor-dce): add support for new Azure feature
feat(monitor-dce): implement additional configuration options
```
...
```bash
fix(monitor-dce): correct validation for resource names
perf(monitor-dce): optimize resource creation logic
docs(monitor-dce): update README with new examples
```
...
```bash
chore(monitor-dce): update .gitignore
test(monitor-dce): add unit tests for validation
```
...
- Commits include module scope: `feat(monitor-dce): description`
...
module "monitor_data_collection_endpoint" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azurerm_monitor_data_collection_endpoint?ref=DCEv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| DCEv1.0.x | >= 1.12.2 | 4.57.0 (pinned) | TBD |
...
```hcl
module "monitor_data_collection_endpoint" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/azurerm_monitor_data_collection_endpoint?ref=DCEv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=DCEv1.2.3`)
...
feat(monitor-dce): add new functionality
fix(monitor-dce): resolve validation issue
...
feat(monitor-dce): expose ingestion endpoints in outputs

Adds configuration/logs/metrics ingestion endpoint outputs.
Improves module usability in monitoring pipelines.

Closes #156

# Breaking change with migration guide
feat(monitor-dce)!: restructure input variables
