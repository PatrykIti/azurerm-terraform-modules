...
```
LINUXVMv{major}.{minor}.{patch}
```

Where:
- `LINUXVM` = azurerm_linux_virtual_machine module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `LINUXVMv1.0.0` - First stable release
- `LINUXVMv1.1.0` - Minor feature addition (from `feat:` commits)
- `LINUXVMv1.0.1` - Bug fix (from `fix:` commits)
- `LINUXVMv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(linux-virtual-machine)!: change default behavior
...
```bash
feat(linux-virtual-machine): add support for new Azure feature
feat(linux-virtual-machine): implement additional configuration options
```
...
```bash
fix(linux-virtual-machine): correct validation for resource names
perf(linux-virtual-machine): optimize resource creation logic
docs(linux-virtual-machine): update README with new examples
```
...
```bash
chore(linux-virtual-machine): update .gitignore
test(linux-virtual-machine): add unit tests for validation
```
...
- Commits include module scope: `feat(linux-virtual-machine): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `LINUXVMv1.2.0`)
...
module "linux_virtual_machine" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azurerm_linux_virtual_machine?ref=LINUXVMv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| LINUXVMv1.0.x | >= 1.12.2 | 4.57.0 (pinned) | TBD |
...
```hcl
module "linux_virtual_machine" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/azurerm_linux_virtual_machine?ref=LINUXVMv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=LINUXVMv1.2.3`)
...
feat(linux-virtual-machine): add new functionality
fix(linux-virtual-machine): resolve validation issue
...
feat(linux-virtual-machine): add support for private endpoints
...
feat(linux-virtual-machine)!: restructure input variables
