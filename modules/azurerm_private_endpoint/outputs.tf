output "id" {
  ...
  value       = try(azurerm_private_endpoint.private_endpoint.id, null)
}

output "name" {
  description = "The name of the Private Endpoint."
  value       = try(azurerm_private_endpoint.private_endpoint.name, null)
}

output "resource_group_name" {
  description = "The name of the resource group containing the Private Endpoint."
  value       = try(azurerm_private_endpoint.private_endpoint.resource_group_name, null)
}

output "location" {
  description = "The Azure region where the Private Endpoint exists."
  value       = try(azurerm_private_endpoint.private_endpoint.location, null)
}

output "subnet_id" {
  description = "The subnet ID from which the Private Endpoint IP address is allocated."
  value       = try(azurerm_private_endpoint.private_endpoint.subnet_id, null)
}

output "custom_network_interface_name" {
  description = "The custom network interface name, if configured."
  value       = try(azurerm_private_endpoint.private_endpoint.custom_network_interface_name, null)
}

output "network_interface" {
  description = "The network interface details created for the Private Endpoint."
  value       = try(azurerm_private_endpoint.private_endpoint.network_interface, [])
}

output "private_service_connection" {
  description = "The private service connection configuration and computed values."
  value       = try(azurerm_private_endpoint.private_endpoint.private_service_connection, [])
}
