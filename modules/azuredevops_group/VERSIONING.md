...
```
ADOGv{major}.{minor}.{patch}
```

Where:
- `ADOG` = azuredevops_group module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `ADOGv1.0.0` - First stable release
- `ADOGv1.1.0` - Minor feature addition (from `feat:` commits)
- `ADOGv1.0.1` - Bug fix (from `fix:` commits)
- `ADOGv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(azuredevops-group)!: change default behavior
...
```bash
feat(azuredevops-group): add support for new Azure feature
feat(azuredevops-group): implement additional configuration options
```
...
```bash
fix(azuredevops-group): correct validation for resource names
perf(azuredevops-group): optimize resource creation logic
docs(azuredevops-group): update README with new examples
```
...
```bash
chore(azuredevops-group): update .gitignore
test(azuredevops-group): add unit tests for validation
```
...
- Commits include module scope: `feat(azuredevops-group): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `ADOGv1.2.0`)
...
module "azuredevops_group" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azuredevops_group?ref=ADOGv1.2.0"
}
```
...
|----------------|-------------------|-----------------------|-------|
| ADOGv1.0.x | >= 1.12.2 | 1.12.2 (pinned) | Aligns with module guide baseline |
...
```hcl
module "azuredevops_group" {
  source = "git::https://github.com/yourusername/azurerm-terraform-modules//modules/azuredevops_group?ref=ADOGv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=ADOGv1.2.3`)
...
feat(azuredevops-group): add new functionality
fix(azuredevops-group): resolve validation issue
...
feat(azuredevops-group): add support for private endpoints
...
feat(azuredevops-group)!: restructure input variables
