...
Manages one Azure DevOps group with optional strict-child memberships.

> Group entitlements were moved to the dedicated module `modules/azuredevops_group_entitlement`.
...
<!-- BEGIN_EXAMPLES -->
- [Basic](examples/basic) - This example demonstrates a minimal Azure DevOps group configuration.
- [Complete](examples/complete) - This example demonstrates group creation with membership composition.
- [Secure](examples/secure) - This example demonstrates a security-focused group configuration with explicit membership overwrite mode.
<!-- END_EXAMPLES -->
...
| <a name="input_group_memberships"></a> [group\_memberships](#input\_group\_memberships) | List of membership assignments for the module-managed group. Each item must include a unique key and at least one member descriptor. | <pre>list(object({<br/>    key                = string<br/>    member_descriptors = list(string)<br/>    mode               = optional(string, "add")<br/>  }))</pre> | `[]` | no |
