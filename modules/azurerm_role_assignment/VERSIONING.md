...
```
RAv{major}.{minor}.{patch}
```

Where:
- `RA` = azurerm_role_assignment module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `RAv1.0.0` - First stable release
- `RAv1.1.0` - Minor feature addition (from `feat:` commits)
- `RAv1.0.1` - Bug fix (from `fix:` commits)
- `RAv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(role-assignment)!: change default behavior
...
```bash
feat(role-assignment): add support for new Azure feature
feat(role-assignment): implement additional configuration options
```
...
```bash
fix(role-assignment): correct validation for resource names
perf(role-assignment): optimize resource creation logic
docs(role-assignment): update README with new examples
```
...
```bash
chore(role-assignment): update .gitignore
test(role-assignment): add unit tests for validation
```
...
- Commits include module scope: `feat(role-assignment): description`
...
module "role_assignment" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azurerm_role_assignment?ref=RAv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| RAv1.0.x | >= 1.12.2 | 4.57.0 (pinned) | TBD |
...
```hcl
module "role_assignment" {
  source = "github.com/yourusername/azurerm-terraform-modules//modules/azurerm_role_assignment?ref=RAv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=RAv1.2.3`)
...
feat(role-assignment): add new functionality
fix(role-assignment): resolve validation issue
...
feat(role-assignment): add support for private endpoints
...
feat(role-assignment)!: restructure input variables
