...
resource "azurerm_resource_group" "example" {
  name     = "rg-aiwb-secure-${var.random_suffix}"
  location = var.location
}

resource "azurerm_log_analytics_workspace" "example" {
  name                = "law-aiwb-secure-${var.random_suffix}"
...
resource "azurerm_user_assigned_identity" "workbook" {
  name                = "uai-aiwb-secure-${var.random_suffix}"
  location            = azurerm_resource_group.example.location
  resource_group_name = azurerm_resource_group.example.name
}
...
module "application_insights_workbook" {
  ...
  resource_group_name = azurerm_resource_group.example.name
  location            = azurerm_resource_group.example.location
  display_name        = "Workbook - Secure Test"
