# Core Azure DevOps Repository Variables
variable "name" {
  description = "The name of the azuredevops_repository. Must be globally unique."
  type        = string

  validation {
    condition     = can(regex("^[a-z0-9]{3,24}$", var.name))
    error_message = "Azure DevOps Repository name must be between 3 and 24 characters long and use numbers and lower-case letters only."
  }
}

variable "resource_group_name" {
  description = "The name of the resource group in which to create the azuredevops_repository."
  type        = string
}

variable "location" {
  description = "The Azure Region where the Azure DevOps Repository should exist."
  type        = string
}
...
variable "security_settings" {
  description = "Security configuration for the azuredevops_repository."
  type = object({
...
variable "network_rules" {
  description = "Network rules configuration for the azuredevops_repository."
  type = object({
...
variable "private_endpoints" {
  description = "List of private endpoints to create for the azuredevops_repository."
  type = list(object({
    name                            = string
    subnet_id                       = string
    subresource_names               = optional(list(string), ["azuredevops_repository"])
