# Azure Azure DevOps Repository Module - Initial Release
resource "azurerm_azuredevops_repository" "main" {
  name                = var.name
...
resource "azurerm_azuredevops_repository_network_rules" "main" {
  count = var.network_rules != null ? 1 : 0

  # TODO: Configure network rules based on resource type
  # storage_account_id = azurerm_azuredevops_repository.main.id
...
resource "azurerm_private_endpoint" "main" {
  ...
  private_service_connection {
    ...
    private_connection_resource_id = azurerm_azuredevops_repository.main.id
...
resource "azurerm_monitor_diagnostic_setting" "main" {
  ...
  target_resource_id             = azurerm_azuredevops_repository.main.id
