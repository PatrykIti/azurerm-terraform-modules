# Contributing to azuredevops_artifacts_feed Module
...
6. Commit using conventional commits: `feat(azuredevops-artifacts-feed): add new feature`
...
```bash
feat(azuredevops-artifacts-feed): add support for new feature
fix(azuredevops-artifacts-feed): correct validation logic
docs(azuredevops-artifacts-feed): update README examples
```
...
2. Run tests locally before submitting PR:
   ```bash
   cd modules/azuredevops_artifacts_feed/tests
   make test
   ```
