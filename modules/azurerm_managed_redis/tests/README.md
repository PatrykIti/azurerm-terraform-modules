# Managed Redis Module Tests

This directory contains automated tests for the Managed Redis Terraform module using [Terratest](https://terratest.gruntwork.io/).
...
```bash
go test -v -run TestBasicManagedRedis -timeout 30m
```
...
- `managed_redis_test.go` - Main module functionality tests
...
- `fixtures/geo-replication/` - Geo-replication configuration
...
- Geo-replication membership validation
...
```bash
go test -v -run TestBasicManagedRedis
```
