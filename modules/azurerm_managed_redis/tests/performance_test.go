...
// BenchmarkManagedRedisCreationSimple benchmarks basic Managed Redis creation.
func BenchmarkManagedRedisCreationSimple(b *testing.B) {
	b.ReportAllocs()
...
// BenchmarkManagedRedisCreationWithFeatures benchmarks different Managed Redis SKUs.
func BenchmarkManagedRedisCreationWithFeatures(b *testing.B) {
	b.ReportAllocs()
	...
	featureConfigs := []struct {
		...
		{name: "Balanced_B3", skuName: "Balanced_B3"},
		{name: "Balanced_B5", skuName: "Balanced_B5"},
	}
...
// BenchmarkManagedRedisCreationWithScale benchmarks scaling scenarios.
func BenchmarkManagedRedisCreationWithScale(b *testing.B) {
	b.Skip("Scaling benchmarks are not applicable for a single Managed Redis resource")
}

// BenchmarkManagedRedisParallelCreation benchmarks parallel creation of Managed Redis.
func BenchmarkManagedRedisParallelCreation(b *testing.B) {
...
// TestManagedRedisCreationTime validates basic creation time.
func TestManagedRedisCreationTime(t *testing.T) {
	if testing.Short() {
	...
	require.LessOrEqual(t, duration, maxDuration,
		"Managed Redis creation took %v, expected less than %v", duration, maxDuration)
}

// TestManagedRedisScaling tests creating multiple Managed Redis instances sequentially.
func TestManagedRedisScaling(t *testing.T) {
	if testing.Short() {
...
// TestManagedRedisUpdatePerformance tests update performance for a lightweight change.
func TestManagedRedisUpdatePerformance(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping performance test in short mode")
	}
	t.Parallel()
...
// TestManagedRedisDestroyPerformance tests destroy performance.
func TestManagedRedisDestroyPerformance(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping performance test in short mode")
	}
	t.Parallel()
