# Contributing to azuredevops_variable_groups Module
...
6. Commit using conventional commits: `feat(azuredevops-variable-groups): add new feature`
...
```bash
feat(azuredevops-variable-groups): add support for new feature
fix(azuredevops-variable-groups): correct validation logic
docs(azuredevops-variable-groups): update README examples
```
...
2. Run tests locally before submitting PR:
   ```bash
   cd modules/azuredevops_variable_groups/tests
   make test
   ```
