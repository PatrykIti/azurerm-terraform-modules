# Contributing to azuredevops_team Module
...
6. Commit using conventional commits: `feat(azuredevops-team): add new feature`
...
```bash
feat(azuredevops-team): add support for new feature
fix(azuredevops-team): correct validation logic
docs(azuredevops-team): update README examples
```
...
2. Run tests locally before submitting PR:
   ```bash
   cd modules/azuredevops_team/tests
   make test
   ```
