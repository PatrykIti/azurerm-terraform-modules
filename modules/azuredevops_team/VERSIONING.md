...
```
ADOTv{major}.{minor}.{patch}
```

Where:
- `ADOT` = azuredevops_team module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `ADOTv1.0.0` - First stable release
- `ADOTv1.1.0` - Minor feature addition (from `feat:` commits)
- `ADOTv1.0.1` - Bug fix (from `fix:` commits)
- `ADOTv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(azuredevops-team)!: change default behavior
...
```bash
feat(azuredevops-team): add support for new Azure feature
feat(azuredevops-team): implement additional configuration options
```
...
```bash
fix(azuredevops-team): correct validation for resource names
perf(azuredevops-team): optimize resource creation logic
docs(azuredevops-team): update README with new examples
```
...
```bash
chore(azuredevops-team): update .gitignore
test(azuredevops-team): add unit tests for validation
```
...
- Commits include module scope: `feat(azuredevops-team): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `ADOTv1.2.0`)
...
module "azuredevops_team" {
  source = "github.com/org/repo//modules/azuredevops_team?ref=ADOTv1.2.0"
}
```
...
|----------------|-------------------|------------------|-------------------|
| ADOTv1.0.x | >= 1.3.0 | 4.36.0 (pinned) | TBD |
...
- **Production**: Always use specific version tags (e.g., `ref=ADOTv1.2.3`)
...
feat(azuredevops-team): add new functionality
fix(azuredevops-team): resolve validation issue
...
feat(azuredevops-team): add support for private endpoints
...
feat(azuredevops-team)!: restructure input variables
