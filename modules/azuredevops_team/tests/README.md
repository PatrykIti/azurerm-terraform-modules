# Azure DevOps Team Module Tests

This directory contains automated tests for the Azure DevOps Team Terraform module using [Terratest](https://terratest.gruntwork.io/).
...
4. **Azure DevOps Project**: Project ID for team creation
...
```bash
go test -v -run TestBasicAzuredevopsTeam -timeout 30m
```
...
- `azuredevops_team_test.go` - Basic, complete, secure, and validation tests
...
```bash
go test -v -run TestBasicAzuredevopsTeam
```
...
3. **Resource Conflicts**: Ensure unique team naming
