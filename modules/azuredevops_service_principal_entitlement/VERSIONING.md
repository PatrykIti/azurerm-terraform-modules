...
```
ADOSPEv{major}.{minor}.{patch}
```

Where:
- `ADOSPE` = azuredevops_service_principal_entitlement module identifier
- `v` = version prefix
- `{major}.{minor}.{patch}` = semantic version numbers (automatically determined)

### Examples:
- `ADOSPEv1.0.0` - First stable release
- `ADOSPEv1.1.0` - Minor feature addition (from `feat:` commits)
- `ADOSPEv1.0.1` - Bug fix (from `fix:` commits)
- `ADOSPEv2.0.0` - Breaking change (from `BREAKING CHANGE:` commits)
...
```bash
feat(azuredevops-service-principal-entitlement)!: change default behavior
...
```bash
feat(azuredevops-service-principal-entitlement): add support for new Azure feature
feat(azuredevops-service-principal-entitlement): implement additional configuration options
```
...
```bash
fix(azuredevops-service-principal-entitlement): correct validation for resource names
perf(azuredevops-service-principal-entitlement): optimize resource creation logic
docs(azuredevops-service-principal-entitlement): update README with new examples
```
...
```bash
chore(azuredevops-service-principal-entitlement): update .gitignore
test(azuredevops-service-principal-entitlement): add unit tests for validation
```
...
- Commits include module scope: `feat(azuredevops-service-principal-entitlement): description`
...
3. **Semantic-release automatically**:
   ...
   - Creates git tag (e.g., `ADOSPEv1.2.0`)
...
module "azuredevops_service_principal_entitlement" {
  source = "git::https://github.com/PatrykIti/azurerm-terraform-modules//modules/azuredevops_service_principal_entitlement?ref=ADOSPEv1.2.0"
}
```

## Version Compatibility Matrix

| Module Version | Terraform Version | Azure DevOps Provider | Notes |
|----------------|-------------------|-----------------------|-------|
| ADOSPEv1.0.x | >= 1.12.2 | 1.12.2 (pinned) | Aligns with module guide baseline |
...
```hcl
module "azuredevops_service_principal_entitlement" {
  source = "git::https://github.com/yourusername/azurerm-terraform-modules//modules/azuredevops_service_principal_entitlement?ref=ADOSPEv1.0.0"
...
- **Production**: Always use specific version tags (e.g., `ref=ADOSPEv1.2.3`)
...
feat(azuredevops-service-principal-entitlement): add new functionality
fix(azuredevops-service-principal-entitlement): resolve validation issue
...
feat(azuredevops-service-principal-entitlement): add support for private endpoints
...
feat(azuredevops-service-principal-entitlement)!: restructure input variables
